...
use rustypotato::{
    HealthChecker, LogRotationManager, MetricsCollector, 
    MonitoringServer, MemoryStore
};
...
async fn test_health_check_with_storage_operations() {
    ...
    metrics.record_command_latency("GET", Duration::from_micros(50)).await;
...
async fn test_log_rotation_cleanup() {
    ...
    assert!(rotated_count <= 2, "Too many rotated files: {}", rotated_count);
}
...
async fn test_metrics_collection_accuracy() {
    ...
    metrics.record_command_latency("SET", Duration::from_micros(100)).await;
    metrics.record_command_latency("GET", Duration::from_micros(50)).await;
    metrics.record_command_latency("SET", Duration::from_micros(120)).await;
...
async fn test_health_check_component_details() {
    ...
    metrics.record_command_latency("TEST", Duration::from_micros(75)).await;
    ...
    assert!(memory_component.details.contains_key("current_memory_bytes"));
    ...
    assert!(network_component.details.contains_key("connection_rejection_rate_percent"));
...
async fn test_log_rotation_status_tracking() {
    ...
    assert_eq!(initial_status.current_file_size, "Initial content".len() as u64);
