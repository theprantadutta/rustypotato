...
async fn test_server_command_integration() {
    ...
    let response = send_command(&mut stream, "*2\r\n$3\r\nGET\r\n$7\r\ntestkey\r\n").await
        .expect("Failed to send GET command");
    assert_eq!(parse_bulk_response(&response), Some("testvalue".to_string()));
        ...
        Ok(Ok(mut stream)) => {
            ...
            if let Ok(n) = timeout(Duration::from_secs(1), stream.read(&mut buffer)).await {
                if let Ok(n) = n {
                    ...
                    assert!(response.contains("connection limit"), 
...
"#);
    ...
    std::env::set_var("RUSTYPOTATO_CONFIG_PATH", config_path.to_string_lossy().to_string());
    ...
    if !build_output.status.success() {
        panic!("Failed to build server binary: {}", 
