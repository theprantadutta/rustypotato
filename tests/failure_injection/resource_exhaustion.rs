...
async fn test_large_value_100kb() {
    ...
    let response = send_and_receive(&mut stream, &["SET", "large_key", &large_value])
        .await
        .unwrap();
    ...
    let response = send_and_receive(&mut stream, &["EXISTS", "large_key"])
        .await
        .unwrap();
...
async fn test_multiple_medium_values() {
    ...
    for i in 0..50 {
        let key = format!("medium_key_{}", i);
        let response = send_and_receive(&mut stream, &["EXISTS", &key])
            .await
            .unwrap();
        assert_eq!(response, b":1\r\n", "Key {} should exist", i);
    }
...
async fn test_hash_many_fields() {
    ...
    for i in 0..500 {
        ...
        let value = format!("value_{}", i);
        let response = send_and_receive(&mut stream, &["HSET", "big_hash", &field, &value])
            .await
            .unwrap();
    ...
    let response = send_and_receive(&mut stream, &["HGET", "big_hash", "field_250"])
        .await
        .unwrap();
    assert_eq!(response, b"$9\r\nvalue_250\r\n");

    let response = send_and_receive(&mut stream, &["HEXISTS", "big_hash", "field_499"])
        .await
        .unwrap();
    assert_eq!(response, b":1\r\n");
}
...
async fn test_concurrent_incr() {
    ...
    let mut stream = TcpStream::connect(addr).await.unwrap();
    let response = send_and_receive(&mut stream, &["GET", "shared_counter"])
        .await
        .unwrap();
...
async fn test_store_concurrent_operations() {
    ...
    for i in 0..100 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            store
                .incr(&format!("store_counter_{}", i % 10))
                .await
                .unwrap();
        }));
    }
    ...
    for i in 0..20 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            for j in 0..50 {
                ...
                // Create
                store
                    .set(key.clone(), "temp_value".to_string())
                    .await
                    .unwrap();
...
async fn test_incremental_hash_growth() {
    ...
    for i in 0..100 {
        ...
        let response = send_and_receive(&mut stream, &["HSET", "growing_hash", &field, &value])
            .await
            .unwrap();
