...
async fn test_command_array_encoding() {
    ...
    let encoded = client.encode_command_array(&["GET".to_string(), "key".to_string()]).unwrap();
...
async fn test_command_array_encoding_with_empty_string() {
    ...
    let encoded = client.encode_command_array(&[
        "SET".to_string(), 
        "key".to_string(), 
        "".to_string()
    ]).unwrap();
...
async fn test_cli_client_basic_operations() {
    ...
    // Test SET command
    let result = client.execute_command("SET", &["test_key".to_string(), "test_value".to_string()]).await;
    assert!(result.is_ok());
    
    // Test GET command
    let result = client.execute_command("GET", &["test_key".to_string()]).await;
    assert!(result.is_ok());
    ...
    // Test EXISTS command
    let result = client.execute_command("EXISTS", &["test_key".to_string()]).await;
    assert!(result.is_ok());
    ...
    // Test DEL command
    let result = client.execute_command("DEL", &["test_key".to_string()]).await;
    assert!(result.is_ok());
...
async fn test_cli_client_ttl_operations() {
    ...
    // Set a key
    client.execute_command("SET", &["ttl_key".to_string(), "ttl_value".to_string()]).await.unwrap();
    
    // Set expiration
    let result = client.execute_command("EXPIRE", &["ttl_key".to_string(), "60".to_string()]).await;
    assert!(result.is_ok());
    
    // Check TTL
    let result = client.execute_command("TTL", &["ttl_key".to_string()]).await;
    assert!(result.is_ok());
    ...
    // Clean up
    client.execute_command("DEL", &["ttl_key".to_string()]).await.unwrap();
...
async fn test_cli_client_atomic_operations() {
    ...
    let result = client.execute_command("INCR", &["counter".to_string()]).await;
    assert!(result.is_ok());
    ...
    // Test DECR
    let result = client.execute_command("DECR", &["counter".to_string()]).await;
    assert!(result.is_ok());
    if let Ok(ResponseValue::Integer(value)) = result {
        assert_eq!(value, 1);
    }
    ...
    // Clean up
    client.execute_command("DEL", &["counter".to_string()]).await.unwrap();
    client.disconnect().await.unwrap();
}
...
async fn test_cli_client_error_handling() {
    ...
    // Test wrong arity
    let result = client.execute_command("SET", &["only_key".to_string()]).await;
    assert!(result.is_ok());
    if let Ok(ResponseValue::SimpleString(msg)) = result {
        assert!(msg.contains("ERR"));
    }
...
async fn test_cli_client_large_value() {
    ...
    let result = client.execute_command("SET", &["large_key".to_string(), large_value.clone()]).await;
    assert!(result.is_ok());
    
    let result = client.execute_command("GET", &["large_key".to_string()]).await;
    assert!(result.is_ok());
    if let Ok(ResponseValue::BulkString(Some(value))) = result {
    ...
    // Clean up
    client.execute_command("DEL", &["large_key".to_string()]).await.unwrap();
    client.disconnect().await.unwrap();
}
...
async fn test_cli_client_special_characters() {
    ...
    let result = client.execute_command("SET", &["special_key".to_string(), special_value.to_string()]).await;
    assert!(result.is_ok());
    
    let result = client.execute_command("GET", &["special_key".to_string()]).await;
    assert!(result.is_ok());
    if let Ok(ResponseValue::BulkString(Some(value))) = result {
    ...
    // Clean up
    client.execute_command("DEL", &["special_key".to_string()]).await.unwrap();
    client.disconnect().await.unwrap();
}
...
async fn test_cli_client_unicode() {
    ...
    let result = client.execute_command("SET", &["unicode_key".to_string(), unicode_value.to_string()]).await;
    assert!(result.is_ok());
    
    let result = client.execute_command("GET", &["unicode_key".to_string()]).await;
    assert!(result.is_ok());
    if let Ok(ResponseValue::BulkString(Some(value))) = result {
    ...
    // Clean up
    client.execute_command("DEL", &["unicode_key".to_string()]).await.unwrap();
    client.disconnect().await.unwrap();
}
...
mod performance_tests {
    ...
    async fn test_cli_client_performance() {
        ...
        for i in 0..num_operations {
            ...
            client.execute_command("SET", &[key.clone(), value]).await.unwrap();
        ...
        println!("Performed {} operations in {:?} ({:.2} ops/sec)", 
                 num_operations * 3, duration, ops_per_sec);
