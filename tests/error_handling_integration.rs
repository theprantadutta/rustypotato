...
async fn create_test_server_with_config(
    config: Config,
) -> (RustyPotatoServer, std::net::SocketAddr) {
...
async fn test_large_command_error_handling() {
    ...
    let result = timeout(
        ...
        send_command(&mut stream, large_cmd.as_bytes()),
    )
    .await;
