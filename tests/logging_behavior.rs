...
async fn test_metrics_collector_comprehensive() {
    ...
    collector
        ...
        .await;
    collector
        .record_command_latency("DEL", Duration::from_micros(80))
        .await;
...
async fn test_log_rotation_cleanup() {
    ...
    assert!(
        status.rotated_files_count <= 3,
        "Should keep approximately max_files rotated files, got {}",
        status.rotated_files_count
    );
}
