...
pub use health::{ComponentHealth, HealthCheckResult, HealthChecker, HealthStatus};
pub use log_rotation::{LogRotationConfig, LogRotationManager, RotationPolicy};
...
impl MonitoringServer {
    ...
    async fn handle_health_check(health_checker: &HealthChecker) -> String {
        ...
        let json_response =
            serde_json::to_string_pretty(&health_result.status).unwrap_or_else(|_| {
                r#"{"status": "error", "message": "Failed to serialize health check"}"#.to_string()
            });
    ...
    async fn format_prometheus_metrics(collector: &MetricsCollector) -> String {
        ...
        output.push_str(&format!(
            "rustypotato_info{{version=\"{}\",build=\"{}\"}} 1\n",
        ...
        if let Some(_avg_latency) = summary.average_latency {
            ...
            output.push_str(&format!(
                "rustypotato_latency_seconds_sum {}\n",
                performance.latency_histogram.sum().as_secs_f64()
            ));
            output.push_str(&format!(
                "rustypotato_latency_seconds_count {}\n",
            ...
            for p in &percentiles {
                if let Some(latency) = performance.latency_histogram.percentile(*p) {
                    output.push_str(&format!(
                        "rustypotato_latency_seconds{{quantile=\"{:.3}\"}} {}\n",
                        p / 100.0,
                        latency.as_secs_f64()
                    ));
                }
