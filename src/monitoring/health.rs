...
impl HealthChecker {
    ...
    pub async fn check_all_components(&self) -> HealthCheckResult {
        ...
        debug!("Health check completed in {:?}, status: {}", 
               start_time.elapsed(), overall_status);
                ...
                match self.storage.get(test_key) {
                    Ok(Some(value)) => {
                        if value.value.to_string() == test_value {
                            ...
                            details.insert("test_operation".to_string(), "value_mismatch".to_string());
                            ComponentHealth {
    ...
    async fn check_metrics_health(&self) -> ComponentHealth {
        ...
        details.insert("total_operations".to_string(), summary.total_operations.to_string());
        details.insert("uptime_seconds".to_string(), summary.uptime.as_secs().to_string());
        details.insert("active_connections".to_string(), summary.active_connections.to_string());
    ...
    async fn check_memory_health(&self) -> ComponentHealth {
        ...
        details.insert("current_memory_bytes".to_string(), current_memory.to_string());
        ...
        let status = if let Ok(sys_info) = sys_info::mem_info() {
            ...
            details.insert("total_system_memory_bytes".to_string(), total_memory.to_string());
            details.insert("memory_usage_percent".to_string(), format!("{:.2}", memory_usage_percent));
    ...
    async fn check_network_health(&self) -> ComponentHealth {
        ...
        details.insert("active_connections".to_string(), network_metrics.active_connections.to_string());
        details.insert("total_bytes_read".to_string(), network_metrics.total_bytes_read.to_string());
        details.insert("total_bytes_written".to_string(), network_metrics.total_bytes_written.to_string());
        details.insert("connections_accepted".to_string(), network_metrics.connections_accepted.to_string());
        details.insert("connections_rejected".to_string(), network_metrics.connections_rejected.to_string());
        
        // Calculate rejection rate
        let total_connection_attempts = network_metrics.connections_accepted + network_metrics.connections_rejected;
        ...
        details.insert("connection_rejection_rate_percent".to_string(), format!("{:.2}", rejection_rate));
