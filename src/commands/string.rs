...
impl Command for SetCommand {
    async fn execute(&self, args: &[String], store: &MemoryStore) -> CommandResult {
        if args.len() != 2 {
            return CommandResult::Error("ERR wrong number of arguments for 'SET' command".to_string());
        }
...
impl Command for GetCommand {
    async fn execute(&self, args: &[String], store: &MemoryStore) -> CommandResult {
        if args.len() != 1 {
            return CommandResult::Error("ERR wrong number of arguments for 'GET' command".to_string());
        }
        ...
        match store.get(key) {
            Ok(Some(stored_value)) => {
                CommandResult::Ok(ResponseValue::BulkString(Some(stored_value.value.to_string())))
            }
...
impl Command for DelCommand {
    async fn execute(&self, args: &[String], store: &MemoryStore) -> CommandResult {
        if args.is_empty() {
            return CommandResult::Error("ERR wrong number of arguments for 'DEL' command".to_string());
        }
...
impl Command for ExistsCommand {
    async fn execute(&self, args: &[String], store: &MemoryStore) -> CommandResult {
        if args.is_empty() {
            return CommandResult::Error("ERR wrong number of arguments for 'EXISTS' command".to_string());
        }
...
mod tests {
    ...
    async fn test_set_command_overwrite() {
        ...
        assert!(matches!(result1, CommandResult::Ok(ResponseValue::SimpleString(_))));
        
        // Overwrite with new value
        let args2 = vec!["key1".to_string(), "value2".to_string()];
        let result2 = cmd.execute(&args2, &store).await;
        assert!(matches!(result2, CommandResult::Ok(ResponseValue::SimpleString(_))));
