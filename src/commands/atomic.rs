...
impl Command for IncrCommand {
    async fn execute(&self, args: &[String], store: &MemoryStore) -> CommandResult {
        if args.is_empty() {
            return CommandResult::Error("ERR wrong number of arguments for 'INCR' command".to_string());
        }
...
impl Command for DecrCommand {
    async fn execute(&self, args: &[String], store: &MemoryStore) -> CommandResult {
        if args.is_empty() {
            return CommandResult::Error("ERR wrong number of arguments for 'DECR' command".to_string());
        }
...
mod tests {
    ...
    async fn test_incr_decr_sequence() {
        ...
        assert!(matches!(result2, CommandResult::Ok(ResponseValue::Integer(2))));
        
        // DECR
        let result3 = decr_cmd.execute(&["counter".to_string()], &store).await;
        assert!(matches!(result3, CommandResult::Ok(ResponseValue::Integer(1))));
        
        // DECR again
        let result4 = decr_cmd.execute(&["counter".to_string()], &store).await;
        assert!(matches!(result4, CommandResult::Ok(ResponseValue::Integer(0))));
        
        // DECR to negative
        let result5 = decr_cmd.execute(&["counter".to_string()], &store).await;
        assert!(matches!(result5, CommandResult::Ok(ResponseValue::Integer(-1))));
    }
            ...
            for _ in 0..10 {
                ...
                let handle = thread::spawn(move || {
                    ...
                    rt.block_on(async {
                        let cmd = IncrCommand;
                        for _ in 0..10 {
                            let _ = cmd.execute(&["concurrent_key".to_string()], &store_clone).await;
                        }
                    });
                });
                handles.push(handle);
